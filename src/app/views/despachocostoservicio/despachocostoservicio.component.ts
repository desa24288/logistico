...
export class DespachocostoservicioComponent implements OnInit {
  ...
  async validacantidadgrilla(id: number,despacho: DetalleSolicitud){
    ...
    if(this.IdgrillaDevolucion(despacho)>=0){
      ...
      // }else{
        stock= await this._creaService.BuscaStockProd(despacho.meinid, this.FormCreaSolicitud.value.bodcodigo, this.usuario, this.servidor).toPromise()
  ...
  async onBuscarPlantillas(){
    ...
    this._BSModalRef.content.onClose.subscribe((response: any) => {
      if (response == undefined) {
        ...
        this.esacodigo,this.cmecodigo, response.planid, '', '', '', this.FormCreaSolicitud.value.bodcodigo,
        0, '', '', 1,"").subscribe(async response_plantilla => {
