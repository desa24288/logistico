...
export class BodegasComponent implements OnInit {
  ...
  public FormBuscaGrilla: FormGroup;
  ...
  constructor(
    ...
    this.FormBuscaGrilla = this.formBuilder.group({
      codigobusqueda: [{ value: null, disabled: false }, Validators.required]
    });
  ...
  BuscaBodega() {
    ...
    this._BSModalRef.content.onClose.subscribe((response: any) => {
      if (response == undefined) {
        ...
        this._buscabodegasService.listaEstructuraBodegas(this.hdgcodigo, this.cmecodigo, response.codbodega, null,
  ...
  setModalBusquedaBodega() {
    ...
    dtModal = {
      ...
      initialState: {
        ...
        codbodega: this.FormBodegas.value.codigo,
  ...
  validaCod(codigo: any, tipo: number) {
    ...
    //  console.log("Validando", codigo, tipo);


    let indx: any;
    switch (tipo) {
      case 1:
        indx = this._bodega_productos.findIndex(x => x.mameincodmei === codigo, 1);
        if (indx >= 0) {
          return true;
        } else {
          return false;
      ...
      case 2:
        indx = this._bodega_servicios.findIndex(x => x.codservicio === codigo, 1);
        if (indx >= 0) {
          return true;
        } else {
          return false;
      ...
      case 3:
        indx = this._bodega_usuarios.findIndex(x => x.userid === codigo, 1);
        if (indx >= 0) {
          return true;
        } else {
          return false;
      ...
      case 4:
        indx = this._bodega_relacion_bodegas.findIndex(x => x.codbodegaorigen === codigo, 1);
        if (indx >= 0) {
          return true;
        } else {
          return false;
    ...
    this._BSModalRef.content.onClose.subscribe((response: any) => {
      if (response == undefined) { }
        ...
        if (this.validaCod(response.codigo, 1) === true) {
          this.alertSwalError.title = "Artículo ya existe asociado a la bodega";
          this.alertSwalError.show();
  ...
  getBodega(codigobodega: number) {
    ...
    if (this.codbodega === null || this.codbodega === '') {
      ...
      this._buscabodegasService.listaEstructuraBodegas(this.hdgcodigo, this.cmecodigo, parseInt(this.codbodega), '',
  ...
  async buscaProductoGrilla() {
    let indx = 0;
    // codigoproductobuscar = this.FormBuscaGrilla.controls.codigobusqueda.value;
  ...
  ActivaBotonCrear() {
    ...
    if (this.FormBodegas.value.codigo == null && this.FormBodegas.value.nombrebodega != null
  ...
  ActivaBotonAgregarArticulo() {
    ...
    if (this.FormBodegas.value.codigo != null
      && this.FormBodegas.value.nombrebodega != null
      && this.FormBodegas.value.estado != null
      && this.FormBodegas.value.tipobodega != null
      && (this.FormBodegas.value.tipoproducto != null)
    ) {
