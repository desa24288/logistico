...
export class RecepciondevolucionpacienteComponent implements OnInit {
  ...
  constructor(
    ...
    this.FormFiltro = this.formBuilder.group({
      numsolicitud  : [{ value: null, disabled: false }, Validators.required],
      fecsolicitud  : [{ value: null, disabled: false }, Validators.required],
    ...
    this._pacienteService.BuscaSolicitudesDevueltasPaciente(this.servidor,this.hdgcodigo,
      ...
      resp => {
        if(resp[0].mensajes[0].estado){
          ...
          this.alertSwalAlert.title = "Sin Datos.";
          this.alertSwalAlert.text = "No existen solicitudes pendientes de recepción";
          this.alertSwalAlert.show();
          this.loading = false;
          this.arregloSolicitudDevueltaPaginacion = [];
  ...
  SolicitudDevuelta(registro: any){
    ...
    this._BSModalRef.content.onClose.subscribe((Retorno: any) => {
      ...
      if(Retorno.respuesta === "OK" || Retorno === undefined){
  ...
  setModal(registro: any) {
    ...
    dtModal = {
      ...
      initialState: {
        titulo: "Recepción Devoluciones de Pacientes",
  ...
  sortby (opt: string){
    ...
    switch (opt) {
      ...
      case 'fecsolicitud':
        this.arregloSolicitudDevuelta.sort(function (a, b) {
          if (a.fecsolicitud > b.fecsolicitud) {
            return rtn1;
          }
          if (a.fecsolicitud < b.fecsolicitud) {
            return rtn2;
  ...
  filtro(){
    ...
    var fecsolicitud : string = this.FormFiltro.controls.fecsolicitud.value;
    ...
    if(fecsolicitud != null){
      ...
      valida = true;
      listabodegasFiltro.forEach((element, index) => {
        console.log(element.fecsolicitud.slice(0,-9), "===", this.datePipe.transform(fecsolicitud, 'dd-MM-yyyy'));
        if(this.datePipe.transform(element.fecsolicitud.slice(0,-9), 'dd-MM-yyyy') === this.datePipe.transform(fecsolicitud, 'dd-MM-yyyy')){
          listaFiltro.unshift(element);
        }
      });
      listabodegasFiltro = listaFiltro;
