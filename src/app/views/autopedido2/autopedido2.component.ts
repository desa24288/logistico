...
export class Autopedido2Component implements OnInit {
  ...
  cantidadsolicitada(id:number,detalle: DetalleSolicitud) {
    ...
    if(detalle.detallelote != null || detalle.detallelote != undefined){
      ...
      if (detalle.cantsoli > detalle.stockorigen) {
        ...
        this.alertSwalAlert.text = "Cantidad excede el permitido y/o no existe lote";
        this.alertSwalAlert.show();
        // this.errorMsg('Cantidad excede el permitido y/o no existe lote');
        detalle.cantsoli = 0;
        // this.verificalote(detalle);
        ...
        if (detalle.cantsoli <= 0) {
          ...
          this.alertSwalAlert.text = "La cantidad a solicitar debe ser mayor a 0";
          this.alertSwalAlert.show();
    ...
    if (this.DetalleMedicamentos.length) {
      for (var data of this.DetalleMedicamentos) {
        if (data.cantsoli <= 0 || data.cantsoli === null) {
          ...
          if(data.cantsoli > 0){
            stock1=  await this._creaService.BuscaStockProd(data.meinid, this.FormCreaSolicitud.controls.bodcodigo.value, this.usuario, this.servidor).toPromise();
  ...
  async setPlantilla(art: DetallePlantillaBodega) {
    ...
    var stock1 :StockProducto[]
    stock1=  await this._creaService.BuscaStockProd(art.meinid, this.FormCreaSolicitud.value.bodcodigo, this.usuario, this.servidor).toPromise()
  ...
  async setProducto(art: Articulos) {
    ...
    var stock1 :StockProducto[];
    stock1=  await this._creaService.BuscaStockProd(art.mein, this.FormCreaSolicitud.value.bodcodigo, this.usuario, this.servidor).toPromise()
    this.stockbodegasolicitante =stock1[0].stockactual;


    if(this.stockbodegasolicitante <=0){

      this.alertSwalAlert.title = "El producto no tiene stock en la bodega";
      this.alertSwalAlert.show();
    }else{
      var detalleSolicitud = new DetalleSolicitud;
      detalleSolicitud.sodeid = 0;
  ...
  setModalBusquedaPlantilla() {
    ...
    dtModal = {
      ...
      class: 'modal-dialog-centered modal-xl',
      initialState: {
        titulo: 'Búsqueda de Plantilla Bodega - Autopedido',
        hdgcodigo: this.hdgcodigo,
        esacodigo: this.esacodigo,
        cmecodigo: this.cmecodigo,
        tipoplantilla: true,
  ...
  pageChangedmedicamento(event: PageChangedEvent): void {
    ...
    this.DetalleMedicamentosPaginacion = this.DetalleMedicamentos.slice(startItem, endItem);
  }

  pageChangedinsumo(event: PageChangedEvent): void {
    const startItem = (event.page - 1) * event.itemsPerPage;
    const endItem = event.page * event.itemsPerPage;
    this.DetalleInsumosPaginacion = this.DetalleInsumos.slice(startItem, endItem);
  }
  ...
  */
  ...
  confirmadobleSolicitud() {
    ...
    this.FormCreaSolicitud.get('esticod').setValue(this.dataPacienteSolicitud.estadosolicitud);
    this.alertSwal.title = "Solicitudes Dispensadas y Recepcionadas Exitosas";
  ...
  */
  ...
  async validaStock(){
    ...
    var index : number = 0;
    this.DetalleMedicamentos.forEach(async element =>  {
      stock =  await this._creaService.BuscaStockProd(element.meinid, this.FormCreaSolicitud.controls.bodcodigo.value, this.usuario, this.servidor).toPromise()
    ...
    this.DetalleInsumos.forEach(async element =>  {
      stock =  await this._creaService.BuscaStockProd(element.meinid, this.FormCreaSolicitud.controls.bodcodigo.value, this.usuario, this.servidor).toPromise()
        if(element.cantadespachar > stock[0].stockactual){
          if (index === 0) {
            this.respPermiso = "<p>Saldo del Artículo <strong>" + element.codmei + "</strong> es " + stock[0].stockactual + "</p>";
