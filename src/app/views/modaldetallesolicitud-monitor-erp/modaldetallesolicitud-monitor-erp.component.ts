...
export class ModaldetallesolicitudMonitorERPComponent implements OnInit {
  ...
  CargaDatos(){
    ...
    if(this.movimientobod != null){
      this._interfacesService.listarmovimientointerfazbodegas(this.movimientobod).subscribe(
        response => {
          if (response != null) {
    ...
    this._interfacesService.listamovimientointerfaz(this.movimientopac).subscribe(
      response => {
        if(response != null){
          if (response.length != 0) {
