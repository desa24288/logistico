...
import { AlertComponent, PageChangedEvent } from 'ngx-bootstrap';
...
export class BusquedacuentasComponent implements OnInit, AfterViewInit {
  ...
  async guardaParametros() {
    ...
    this.structSession.fechadesde = this.FormBusquedasolicitud.controls.fechadesde.value;
    this.structSession.fechahasta  = this.FormBusquedasolicitud.controls.fechahasta.value;
  ...
  async getCarga() {
    ...
    const fechainicio = await this.datePipe.transform(this.FormBusquedasolicitud.controls.fechadesde.value, 'yyyy-MM-dd');
    const fechatermino = await this.datePipe.transform(this.FormBusquedasolicitud.controls.fechahasta.value, 'yyyy-MM-dd');
    ...
    this.busquedacuentaService.consultaCargoCuenta(this.servidor, this.hdgcodigo, this.esacodigo,
      ...
      fechainicio,  fechatermino).subscribe(resp => {
        if (resp !== null) {
        ...
        this.loading = false;
        this.alertSwalError.title = 'Error en el proceso';
  ...
  async getSolicitudes(value: CargoFarmacia) {
    ...
    this.busquedacuentaService.consultaSolicitudPaciente(this.servidor, this.hdgcodigo, this.esacodigo,
      ...
      fechainicio, fechatermino).subscribe( res => {
        arrsol = res;
        ...
        if (res.length === 1) {
          ...
          this.alertSwalAlert.text = 'Cargo no ingresado en Logistico';
  ...
  setModalCargasolicitudes(cabecera: string, idcuenta: string, numcta: string, codprod: string) {
   ...
   onLimpiar() {
    ...
    this.FormBusquedasolicitud.controls.fechadesde.reset();
    this.FormBusquedasolicitud.controls.fechahasta.reset();
    sessionStorage.removeItem('structsession');
    this.currentpcargos = 1;
  }
  ...
  async onBuscarctasnompaciente(value: BusquedaPacientes) {
    ...
    await this.getPacientes(false);
  }

  async getPacientes(buscarpac: boolean) {
    this.loading = true;
    if (buscarpac === true) {
      ...
      this.busquedacuentaService.consultaPaciente(this.server, this.cmecodigo,
  ...
  async Parametrosproducto() {
    ...
    this.busquedacuentaService.consultaCuentaMasivo(this.server, this.hdgcodigo,
      this.esacodigo, this.cmecodigo, fechainicio, fechatermino,
  ...
  async setFormulario(){
    ...
    if ( this.structSession === undefined || this.structSession === null) {
      ...
      this.FormBusquedasolicitud.controls.fechadesde.setValue(this.structSession.fechadesde);
      this.FormBusquedasolicitud.controls.fechahasta.setValue(this.structSession.fechahasta);
