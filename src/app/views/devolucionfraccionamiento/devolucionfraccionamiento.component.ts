...
export class DevolucionfraccionamientoComponent implements OnInit {
  ...
  BuscaBodegaFraccionable(){
    this._bodegasService.BuscaBodegasFraccionable(this.hdgcodigo, this.esacodigo, this.cmecodigo,
      ...
      alert("Error al Buscar Bodegas Fraccionables");
    });
  }
  ...
  ConfirmaConsultaFraccionamiento(){
    const Swal = require('sweetalert2');
    Swal.fire({
      title: '¿ Desea Consultar Fraccionamiento del Producto?',
      text: "Confirmar la consulta de fraccionamiento del producto ",
      icon: 'question',
      showCancelButton: true,
      confirmButtonColor: '#3085d6',
      cancelButtonColor: '#d33',
      ...
      response => {
        if (response != null) {
          if(response.length == 0){
            this.alertSwalAlert.title ="No existe información para los parámetros ingresados";
  ...
  ConfirmaDevolucionFraccionamiento(){
    const Swal = require('sweetalert2');
    Swal.fire({
      title: '¿ Desea Grabar Devolución Fraccionamiento del Producto?',
      text: "Confirmar devolución del fraccionamiento",
      showCancelButton: true,
      confirmButtonColor: '#3085d6',
      cancelButtonColor: '#d33',
      confirmButtonText: 'Aceptar'
    ...
    await this._bodegasService.DevolucionFraccionamiento(this.servidor,this.usuario,this.hdgcodigo,
      ...
      response => {
        if(response === undefined || response === null){
          this.alertSwalError.title = "No fue posible devolver El fraccionamiento";
          ...
          this.alertSwal.title ="Devolución Fraccionamiento realizado con éxito";
          this.alertSwal.show();
          this.loading = false;
        }
