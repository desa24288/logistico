...
export class PanelIntegracionERPComponent implements OnInit {
  ...
  BuscarMovimientoInterfazERP() {
    ...
    this._interfacesService.listarmovimientointerfazbodegascab(this._MovimientoInterfazBodegas).subscribe(
      response => {
        if (response != null) {
          if (response.length != 0) {
            this.listabodegas = response;
            this.listabodegas_aux = response;
      ...
      this._interfacesService.listamovimientointerfazpacientecab(this._MovimientoInterfazBodegas).subscribe(
        response => {
          if (response != null) {
            if (response.length != 0) {
    ...
    this._solicitudConsumoService.buscarsolicitudconsumocabecera(0, this.hdgcodigo, this.esacodigo, this.cmecodigo, 0, 0, 0, 0, 0, 0, "", "", this.usuario, this.servidor, fechadesde, fechahasta).subscribe(
      respuestasolicitud => {
    ...
    this._interfacesService.buscarsolicitudacentrales(this.hdgcodigo, this.esacodigo, this.cmecodigo,
      this.usuario, this.servidor, fechadesde, fechahasta).subscribe(
      respuestasolicitud => {
        if (respuestasolicitud != null) {
          if (respuestasolicitud.length != 0) {
            this.listasolicitudesacentrales = respuestasolicitud;
  ...
  Enviar_Solicitud(registro: SolicitudConsumo) {
    ...
    const respose = this._interfacesService.wsLogIntegraPedido(_ConsultaSolConsumoERP).subscribe(
      response => {
        if (response == undefined || response === null) {
          this.alertSwal.title = "No hay respuesta del servidor";
          this.alertSwal.show();
          this.loading = false;
          this.refrescar();
          return;
  ...
  filtroPaciente(){
    ...
    var referencia : number = this.FormFiltroPac.controls.referencia.value;
  ...
  sortbyBod(opt: string){
    ...
    switch (opt) {
      case 'id':
        this.listabodegasPaginacion.sort(function (a, b) {
          if (a.id > b.id) {
            return rtn1;
          }
          if (a.id < b.id) {
  ...
  onImprimir(tipoDoc : string,sol : number,solcon : number, tipo : number) {
    const Swal = require('sweetalert2');
    Swal.fire({
      title: '¿Desea Imprimir el Detalle del Movimiento?',
      ...
      if (result.value) {
        this.Imprimir(tipoDoc,sol,solcon,tipo);
      }
    });
  }

  Imprimir(tipoDoc: string,sol : number,solcon : number, tipo : number) {
    var fechadesde = this.datePipe.transform(this.lForm.value.fechadesde, 'yyyy-MM-dd');
    var fechahasta = this.datePipe.transform(this.lForm.value.fechahasta, 'yyyy-MM-dd');
    this._imprimesolicitudService.RPTImprimePanelIntegracionERP(this.servidor, this.hdgcodigo, this.esacodigo,
      this.cmecodigo, this.usuario,tipoDoc,fechadesde,fechahasta,sol,solcon,tipo).subscribe(
        response => {
          if (response != null) {
        ...
        error => {
          this.alertSwalError.title = "Error al Imprimir Detalle del Movimiento";
          this.alertSwalError.show();
  ...
  onImprimirMasivo(tipoDoc : string) {
    const Swal = require('sweetalert2');
    Swal.fire({
      title: '¿Desea Imprimir los Movimientos realizados?',
      text: "Confirmar Impresión",
      icon: 'question',
      showCancelButton: true,
      confirmButtonColor: '#3085d6',
      ...
      if (result.value) {
        this.ImprimirMasivo(tipoDoc);
      }
    });
  }

  ImprimirMasivo(tiporeport : string) {
    this.filtro = new FiltroERP();
    this.filtro.servidor   = this.servidor;
    this.filtro.tiporeport = tiporeport;
    this.filtro.hdgcodigo  = this.hdgcodigo;
    this.filtro.esacodigo  = this.esacodigo;
    this.filtro.cmecodigo  = this.cmecodigo;
    this.filtro.usuario    = this.usuario;
    this.filtro.fechadesde = this.datePipe.transform(this.lForm.value.fechadesde, 'yyyy-MM-dd');
    this.filtro.fechahasta = this.datePipe.transform(this.lForm.value.fechahasta, 'yyyy-MM-dd');
    this.filtro.opcion     = this.selOpc;

    switch (this.selOpc) {
      case "BODEGAS":
        this.filtro.movid               = this.FormFiltroBod.controls.id.value;
        this.filtro.soliid              = this.FormFiltroBod.controls.soliid.value;
        this.filtro.fechasol            = this.FormFiltroBod.controls.fecha.value;
        this.filtro.codbodegasolicita   = this.FormFiltroBod.controls.codbodegasuministro.value;
        this.filtro.codbodegasuministro = this.FormFiltroBod.controls.codbodegasolicitantes.value;
        this.filtro.codestado           = this.FormFiltroBod.controls.estado.value;
        this.filtro.referencia          = this.FormFiltroBod.controls.referencia.value;
        this.filtro.receid              = 0;
        this.filtro.ctanumcuenta        = 0;
        this.filtro.numidentificacion   = "";
        this.filtro.nombrepac           = "";
        this.filtro.codservicio         = "";
        this.filtro.codcentrocosto      = 0;
        this.filtro.descripcion         = "";
        this.filtro.observacion         = "";
        break;
      case "PACIENTES":
        this.filtro.movid               = this.FormFiltroPac.controls.id.value;
        this.filtro.soliid              = this.FormFiltroPac.controls.soliid.value;
        this.filtro.fechasol            = this.FormFiltroPac.controls.fecha.value;
        this.filtro.codbodegasolicita   = 0;
        this.filtro.codbodegasuministro = 0;
        this.filtro.codestado           = this.FormFiltroPac.controls.estado.value;
        this.filtro.referencia          = this.FormFiltroPac.controls.referencia.value;
        this.filtro.receid              = this.FormFiltroPac.controls.receta.value;
        this.filtro.ctanumcuenta        = this.FormFiltroPac.controls.cuenta.value;
        this.filtro.numidentificacion   = this.FormFiltroPac.controls.rut.value;
        this.filtro.nombrepac           = this.FormFiltroPac.controls.paciente.value;
        this.filtro.codservicio         = this.FormFiltroPac.controls.servicio.value;
        this.filtro.codcentrocosto      = 0;
        this.filtro.descripcion         = "";
        this.filtro.observacion         = "";
        break;
      case "SOLICITUDES":
        this.filtro.movid               = 0;
        this.filtro.soliid              = this.FormFiltroSolGS.controls.id.value;
        this.filtro.fechasol            = this.FormFiltroSolGS.controls.fechasolicitud.value;
        this.filtro.codbodegasolicita   = 0;
        this.filtro.codbodegasuministro = 0;
        this.filtro.codestado           = this.FormFiltroSolGS.controls.glosaestado.value;
        this.filtro.referencia          = this.FormFiltroSolGS.controls.referenciacontable.value;
        this.filtro.receid              = 0;
        this.filtro.ctanumcuenta        = 0;
        this.filtro.numidentificacion   = "";
        this.filtro.nombrepac           = "";
        this.filtro.codservicio         = "";
        this.filtro.codcentrocosto      = this.FormFiltroSolGS.controls.glosacentrocosto.value;
        this.filtro.descripcion         = this.FormFiltroSolGS.controls.glosa.value;
        this.filtro.observacion         = this.FormFiltroSolGS.controls.errorerp.value;
        break;
      case "SOLICITUDESACENTRALES":
        this.filtro.movid               = 0;
        this.filtro.soliid              = this.FormFiltroSolBC.controls.id.value;
        this.filtro.fechasol            = this.FormFiltroSolBC.controls.fechacreacion.value;
        this.filtro.codbodegasolicita   = this.FormFiltroSolBC.controls.bodorigendesc.value;
        this.filtro.codbodegasuministro = this.FormFiltroSolBC.controls.boddestinodesc.value;
        this.filtro.codestado           = this.FormFiltroSolBC.controls.estadosolicitudde.value;
        this.filtro.referencia          = this.FormFiltroSolBC.controls.nropedidofin700erp.value;
        this.filtro.receid              = 0;
        this.filtro.ctanumcuenta        = 0;
        this.filtro.numidentificacion   = "";
        this.filtro.nombrepac           = "";
        ...
        this.filtro.codcentrocosto      = 0;
        this.filtro.descripcion         = "";
        this.filtro.observacion         = this.FormFiltroSolBC.controls.errorerp.value;
        break;
      default:
        break;
    }

    this._imprimesolicitudService.RPTImprimePanelIntegracionERPMasivo(this.filtro).subscribe(
        response => {
          if (response != null) {
            window.open(response[0].url, "", "");
          }
        ...
        error => {
          this.alertSwalError.title = "Error al Imprimir los Movimientos realizados";
          this.alertSwalError.show();
          this._BSModalRef.content.onClose.subscribe((RetornoExito: any) => {
          });
        }
